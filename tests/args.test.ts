...
describe("parseArgs", () => {
  it("parses bare command", () => {
    expect(parseArgs(["test"])).toMatchObject({ command: "test", testType: null, dryRun: false, listOnly: false });
  });

  it("parses --type with value", () => {
    expect(parseArgs(["test", "--type", "e2e"])).toMatchObject({ command: "test", testType: "e2e" });
  });
  ...
  it("parses --dry-run and --list-only", () => {
    expect(parseArgs(["test", "--dry-run", "--list-only"])).toMatchObject({ dryRun: true, listOnly: true });
  });
